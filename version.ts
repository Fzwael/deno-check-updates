...
export async function addLatestVersions(dependencies: dependencyType[]): Promise<dependencyType[]> {
  ...
  result = await Promise.all(dependencies.map(async (dependency) => {
    if (database[dependency.module]?.type === 'github') {
      const url = `https://api.github.com/repos/${database[dependency.module].owner}/${database[dependency.module].repo}/releases/latest`;
      ...
      console.error('This module is not supported yet, please report this with dependency name = ', database[dependency.module]);
      console.error('This module is not supported yet, please report this with dependency name = ', dependency.name);
      return dependency;
    }
  }));
  return result;
...
async function getVersionFromUrl(dependency: dependencyType, url: string): Promise<dependencyType> {
  const versionResult = await soxa.get(url)
    .catch((error) => {
